...
const HuellaCarbonoDirector: React.FC = () => {
    ...
    try {
      ...
      if (tipoReporte === 'locatario') {
        const res = await api.get('/reportes/huella/locatario', {
          params: { local_id: selectedLocal, anio }
        });
        data = res.data.data;
      } else {
        const res = await api.get('/reportes/huella/plaza', {
          params: { plaza_id: selectedPlaza, anio }
        });
        data = res.data.data;
      }

      generarReporteHTML(data, tipoReporte);
  ...
  const generarReporteHTML = (data: any, tipo: TipoReporte) => {
    const VERDE = '#1A5C38';
    const VERDE_M = '#2E7D52';
    const VERDE_L = '#8DC63F';
    ...
    const titulo = tipo === 'locatario'
      ? `${data.locatario.nombre}`
      : `${data.plaza.nombre}`;

    const subtitulo = tipo === 'locatario'
      ? `${data.locatario.plaza} · ${data.locatario.giro} · ${data.locatario.ciudad}`
      : `${data.plaza.ciudad}, ${data.plaza.estado}`;

    const periodo = tipo === 'locatario'
      ? `Enero — Diciembre ${data.periodo.anio}`
      : `Enero — Diciembre ${data.periodo.anio}`;

    const resumen = data.resumen;
    const eq = resumen.equivalencias;

    // Materiales reciclables (sin inorgánico)
    ...
    const logoBase64 = ''; // Se carga dinámicamente — ver nota abajo
...
.portada-logo{height:70px;filter:brightness(0) invert(1);}
...
<div class="contenido page">
  ...
  <div class="metodo-grid">
    ...
    <div class="metodo-item">
      ...
      <div class="metodo-texto">Factor de Emisión del Sistema Eléctrico Nacional (FESEN) 2023: 0.454 kg CO₂/kWh. Publicado por CFE/SEMARNAT/INECC. Referencia oficial para cálculos de huella de carbono en México.</div>
      <div class="metodo-url">gob.mx/inecc · gob.mx/semarnat</div>
    </div>
    <div class="metodo-item">
      <div class="metodo-titulo">Equivalencia de árboles</div>
      <div class="metodo-texto">EPA GHG Equivalencies Calculator: 21.77 kg CO₂ absorbidos por árbol adulto por año. Factor de secuestro de carbono en bosques templados de América del Norte.</div>
...
const matKilos = ${JSON.stringify(matsReciclados.map((m: any) => m.kilos))};

new Chart(document.getElementById('chartTendencia'),{
  type:'bar',
  data:{labels:meses,datasets:[
    {label:'CO₂ evitado',data:co2Mes,backgroundColor:'rgba(26,92,56,0.75)',borderColor:'#1A5C38',borderWidth:1,borderRadius:5},
    {label:'Tendencia',data:co2Mes,type:'line',borderColor:'#8DC63F',borderWidth:2,pointBackgroundColor:'#8DC63F',pointRadius:3,tension:0.4,fill:false}
  ]},
  options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false},tooltip:{callbacks:{label:ctx=>' '+ctx.raw.toLocaleString('es-MX')+' kg CO₂'}}},scales:{x:{grid:{display:false},ticks:{font:{size:10}}},y:{ticks:{font:{size:10},callback:v=>(v/1000).toFixed(0)+'k'},grid:{color:'rgba(0,0,0,0.05)'}}}}
});

new Chart(document.getElementById('chartDonut'),{
  type:'doughnut',
  data:{labels:matLabels,datasets:[{data:matData,backgroundColor:matColors,borderWidth:2,borderColor:'#fff'}]},
  options:{responsive:true,maintainAspectRatio:false,cutout:'58%',plugins:{legend:{display:true,position:'right',labels:{font:{size:10},boxWidth:10,padding:6}},tooltip:{callbacks:{label:ctx=>' '+ctx.raw.toLocaleString('es-MX')+' kg'}}}}
});

new Chart(document.getElementById('chartBarras'),{
  type:'bar',
  data:{labels:matLabels,datasets:[{data:matKilos,backgroundColor:matColors,borderRadius:5}]},
  options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{display:false},tooltip:{callbacks:{label:ctx=>' '+ctx.raw.toLocaleString('es-MX')+' kg'}}},scales:{x:{grid:{display:false},ticks:{font:{size:10}}},y:{ticks:{font:{size:10},callback:v=>v>=1000?(v/1000).toFixed(0)+'k':v},grid:{color:'rgba(0,0,0,0.05)'}}}}
});
</script>
</body></html>`;
